...
# Get Container App Environment (will be available as container_app_environment_name after next azd up)
if [ -z "$container_app_environment_name" ]; then
  CONTAINER_APP_ENV=$(az containerapp env list --resource-group $RESOURCE_GROUP --query "[0].name" -o tsv)
else
...
# Get Container App Environment (will be available as container_app_environment_name after next azd up)
if (-not $container_app_environment_name) {
  $CONTAINER_APP_ENV = az containerapp env list --resource-group $RESOURCE_GROUP --query "[0].name" -o tsv
} else {
  $CONTAINER_APP_ENV = $container_app_environment_name
}
