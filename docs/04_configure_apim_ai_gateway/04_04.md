...
1. **Get your subscription keys**:
   - Run `azd env get-values` in the terminal to see the subscription keys, or
...
1. **Interpreting the Cost Analysis**:
   - 🟢 **Green**: Subscription is under 70% of quota (healthy)
   - 🟡 **Yellow**: Subscription is 70-90% of quota (warning)
   - 🔴 **Red**: Subscription exceeded quota (should be disabled by alert)
...
1. **How the alert workflow works**:
   ```
   1. Token consumption logged → Azure Monitor custom table
   2. Alert rule runs query every 5 minutes
   3. Query calculates: actual spending vs. quota
   4. If spending > quota → Alert fires
